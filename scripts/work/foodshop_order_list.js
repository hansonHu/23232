...
$(document).ready(function(){
	...
	$('.entry').css({'overflow-y':'auto','-webkit-overflow-scrolling':'touch',height:$(window).height()-(common.checkIosApp() ? 124 : 104)});
	...
	$(".book .ensure,.mask").click(function() {
...
function showList(scrollIndex){
	...
	if(isSearch == false){
		...
		common.http('Storestaff&a=foodshop_order',{'page':nowPage, keyword:$('#find_value').val(), 'status':urlParam.status,noTip:true}, function(data){
			if(nowPage >= data.totalPage){
				hasMore = false;
				$('.jroll-infinite-tip').addClass('hideText');
			}
